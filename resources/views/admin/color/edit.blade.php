    ...
    <div class="card card-statistics">
        <div class="card-header">
            <div class="card-heading">
                <h4 class="card-title">Cập nhật màu</h4>
            </div>
        </div>
        <div class="card-body">
            <form action="{{ route('color.update', $color->id) }}" method="post" class="form-horizontal">
                @csrf
                @method('put')
                <div class="form-group">
                    ...
                    <div class="mb-2">
                        {{-- <input type="text" class="form-control " name="name"
                            value ="{{ isset($color) ? $color->name : old('name') }}" placeholder="Điền ..." /> --}}
                        <input type="text" class="form-control " oninput="ChangeToSlug()" id="name" name="name"
                            value ="{{ old('name', $color->name) }}" placeholder="Điền ..." />
                        @error('name')
                ...
                <div class="form-group">
                    ...
                    <div class="mb-2">
                        <input type="text" class="form-control" id="convert_slug" name="slug" placeholder="Điền ..."
                            value ="{{ old('slug', $color->slug) }}" readonly />
                        @error('slug')
                            <em class="text-danger" style="">{{ $message }}</em>
                        @enderror
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="mb-2">
                        @php
                            $selected = $color->status == 0 ? 'selected' : '';
                        @endphp
